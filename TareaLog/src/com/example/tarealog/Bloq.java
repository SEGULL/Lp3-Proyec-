...
public class Bloq extends Activity {
	...
	public void validaringreso(View view) {
		...
		UsuarioSQLiteHelper uDB=new UsuarioSQLiteHelper(getApplicationContext(), "DBUsuarios", null, 41);//instancia de la DB
		...
		db.close();
		UsuarioSQLiteHelper uDBj=new UsuarioSQLiteHelper(getApplicationContext(), "DBUsuarios", null, 41);//instancia de la DB
		SQLiteDatabase dbj=uDBj.getWritableDatabase();
		Cursor cursor = dbj.rawQuery("SELECT  User FROM Usuario where act='1' ", null);
		if(!cursor.moveToNext()){
			Intent intent = new Intent(getApplicationContext(), Log.class);		
			startActivity(intent);
		}else{
			Intent inten = new Intent(getApplicationContext(), WelcomeUser.class);		
			startActivity(inten);
		}
}
